"""
Problema 3: Simulación de una ciudad cuadrada con distribución de personas en cluster.
"""
...
df = pd.concat([df, pd.DataFrame({'posición x': [D + 10], 'posición y': [D + 10], 'estado': [
               "Recuperado"], 'id único': [-1], 'iteracion': [0]})], ignore_index=True)

# Crear la Gráfica
plt.figure(figsize=(8, 8))
plt.axhline(-D / 2, color="blue", linestyle="--")
plt.axhline(D / 2, color="blue", linestyle="--")
plt.axvline(-D / 2, color="blue", linestyle="--")
plt.axvline(D / 2, color="blue", linestyle="--")
...
def rutina(df, D, radio_infeccion, razon_recuperacion):
    ...
    for row in df_return.itertuples():
        while (True):
            x = row[1]
            y = row[2]
            sum_x, sum_y = polares_rectangulares(
                random.uniform(0, 2*np.pi), random.uniform(0, 1))
            new_x = x + sum_x
            new_y = y + sum_y
            if ((-D / 2 <= x <= D / 2) and (-D / 2 <= y <= D / 2)):
...
colores = {'Susceptible': 'black', 'Infectado': 'red', 'Recuperado': 'blue'}
for estado, color in colores.items():
    plt.scatter(ultimo_df[ultimo_df['estado'] == estado]['posición x'],
                ultimo_df[ultimo_df['estado'] == estado]['posición y'],
                color=color, s=10, label=f"{estado}", zorder=3)
plt.axhline(0, color='gray', linewidth=0.5)
plt.axvline(0, color='gray', linewidth=0.5)
plt.scatter(cluster_x, cluster_y, color="green",
...
fig = px.scatter(
    ...
    color_discrete_map={"Susceptible": "black",
                        "Infectado": "red", "Recuperado": "blue"},
    range_x=[-D / 2 - 5, D / 2 + 5],
    range_y=[-D / 2 - 5, D / 2 + 5]
)

# Dibujar los Límites del Cuadrado
fig.update_layout(
    shapes=[dict(
        type="rect",
